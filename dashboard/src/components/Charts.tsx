...
export const CheckInChart = React.memo(function CheckInChart({
  ...
  const handleDotClick = useCallback((dotProps: Record<string, unknown>) => {
    ...
    // cx/cy are relative to the SVG; offset by skeuo-inset margin (12px) + panel padding (0) + header (~28px)
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 12, bottom: 4, left: -16 }}>
            ...
            <Area type="monotone" dataKey="value" name="Check-ins" stroke="#0095FF" fill="url(#ciGrad)" strokeWidth={2}
              dot={{ r: 4, fill: "#0095FF", stroke: "none", cursor: "pointer" }}
              activeDot={((props: any) => {  // eslint-disable-line @typescript-eslint/no-explicit-any
                return <circle key={props.key} cx={props.cx} cy={props.cy} r={6} fill="#0095FF" stroke="#fff" strokeWidth={2} cursor="pointer"
                  onClick={(e) => { e.stopPropagation(); handleDotClick(props); }} />;
              }) as any}
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={chartData} margin={{ top: 10, right: 12, bottom: 4, left: -16 }}>
            ...
            <Area type="monotone" dataKey="value" name="Check-outs" stroke="#F7941D" fill="url(#coGrad)" strokeWidth={2}
              dot={{ r: 4, fill: "#F7941D", stroke: "none", cursor: "pointer" }}
              activeDot={((props: any) => {  // eslint-disable-line @typescript-eslint/no-explicit-any
                return <circle key={props.key} cx={props.cx} cy={props.cy} r={6} fill="#F7941D" stroke="#fff" strokeWidth={2} cursor="pointer"
                  onClick={(e) => { e.stopPropagation(); handleDotClick(props); }} />;
              }) as any}
              isAnimationActive={false} />
          </AreaChart>
